# ============================================================
# montar_1080.ps1 - Homenagem Marlene - v03 em 1080p
# ============================================================
...
Write-Host ""
Write-Host "=== Homenagem Marlene - v03 1080p ===" -ForegroundColor Cyan
Write-Host ""
...
function Make-Text {
    param($id, $linha1, $linha2 = "", $dur = 6, $cor = "white", $tam1 = 108, $tam2 = 72)
    $out = "$TEMP\$id.mp4"
    if (Test-Path $out) { return $out }
    ...
    if ($linha2 -ne "") {
        $yc = [int]($H / 2)
        $vf = "drawtext=fontfile='$fonte':text='$linha1':fontcolor=${cor}:fontsize=${tam1}:x=(w-text_w)/2:y=$($yc-75),drawtext=fontfile='$fonte':text='$linha2':fontcolor=0xE8D5A3:fontsize=${tam2}:x=(w-text_w)/2:y=$($yc+30),format=yuv420p"
    } else {
...
$segs += Make-Text   "04_nome"                      "MARLENE"                           "" 5   "white"   135
$segs += Make-Text   "05_sobrenome"                 "Rodrigues da Conceicao"            "" 5   "#E8D5A3" 81
$segs += Make-Text   "06_datas"                     "20 outubro 1955 - 22 junho 2025"   "" 7   "#AAAAAA" 54
...
$segs += Make-Text   "44_frase1"                    "Vai dar certo,"                          "" 4  "white"   96
$segs += Make-Text   "45_frase2"                    "seu fresco!!!"                           "" 5  "#E8D5A3" 108
$segs += Make-Text   "46_legado"                    "Honrar a mae e honrar a origem."         "" 7  "#AAAAAA" 63
$segs += Make-Text   "47_dedicatoria"               "Com amor eterno -" "Mayko e seus filhos"    7  "white"   69  57
$segs += Make-Text   "48_in_memoriam"               "In memoriam" "Marlene Rodrigues da Conceicao" 9 "#E8D5A3" 75 51
...
$output_final = "$OUT\homenagem_marlene_v03_1080p.mp4"
ffmpeg -y `
    -i $video_sem_audio `
    -i $MUSICA `
    -map 0:v -map 1:a `
